    ...
    <div class="bg-white rounded-xl shadow-sm p-6 max-w-2xl">
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                @if($category->image)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $category->image) }}" alt="Current Image" class="w-20 h-20 object-cover rounded-lg">
                    </div>
                @endif
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-[#5C4033] text-white rounded-lg hover:bg-[#4a332a] transition">Update Category</button>
            </div>
        </form>
    </div>
</x-admin-layout>
